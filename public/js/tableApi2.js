...
function table(urlApi, tableId, dataString, paginateId) {
    let datastring = dataString
    let url = urlApi
    let paginateTable = `#${paginateId}`;
    const tableBody = $(`#${tableId}`)
    let htmlTable = "";
    tableBody.html('processing...')
    const formatKeys = ['debet', 'kredit', 'total', 'harga', 'pokok', 'saldo', 'harga_satuan'];
        ...
        success: function (responses) {
            ...
            if (dataQueries.data.length === 0) {
                ...
                $.each(dataQueries.data, function (key, item) {
                    ...
                    htmlTable += "<td class='sort-nama'>" + numb + "</td>"
